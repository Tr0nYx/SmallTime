...
for ($year = $_now; $year >= $_to; $year--) {
    ...
    for ($month = 0; $month < 12; $month++) {
        if ($anzeige[$year]['Ferien'][$month] <> 0) {
            $anzeige[$year]['Ferien'][$month] = $anzeige[$year]['Ferien'][$month]." Tg.";
        }
    ...
    //Jahressummen
    $a = '';
    if ($anzeige[$year]['Summ']['Auszahlung'] <> 0) {
        $a = format($anzeige[$year]['Summ']['Auszahlung'])." h";
    }
    echo "
	<tr>
    ...
    // Vorholzeiten
    if ($anzeige[$year]['Summ']['vorholzeit']) {
        $v = '- '.$anzeige[$year]['Summ']['vorholzeit'];
    }
    // nicht das Startjahr?
    ...
    if ($year != $_jahr->_startjahr) {
        ...
        if ($anzeige[$year - 1]['Summ']['Ferien'] <> 0) {
            $txt = " Tage";
        }
        //Vorjahr
        echo "
