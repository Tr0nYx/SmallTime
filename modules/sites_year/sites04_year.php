...
for ($i = 0; $i < 12; $i++) {
    ...
    $time_month = new time_month($_settings->_array[12][1], $_temp_time->_letzterTag, $_user->_ordnerpfad,
        $_time->_jahr, $i + 1, $_user->_arbeitstage, $_user->_feiertage, $_user->_SollZeitProTag,
...
echo "<tr>";
echo "<td class='alert";
echo $_SummeSaldoProMonat >= 0 ? " alert-success table-success" : " alert-error table-danger";
...
for ($i = 0; $i < 12; $i++) {
    ...
    if ($_jahres_berechnung[$i]->_SummeSaldoProMonat < 0) {
        echo "<font class=minus>";
    }
