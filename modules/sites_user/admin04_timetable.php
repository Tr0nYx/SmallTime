 ...
 *******************************************************************************/
// ----------------------------------------------------------------------------
// Spaltenreite vergr�ssern, wenn Benutzer keine Berechtigungen haben
// ----------------------------------------------------------------------------
$t = 1;
if ($_settings->_array[15][1] || $_settings->_array[26][1]) {
    $t++;
}
if ($_settings->_array[16][1] || $_settings->_array[26][1]) {
    $t++;
}
$a = 1;
...
<div class="table-responsive">
    <table class='table' border=0 cellpadding=3 cellspacing=1>
        <tr>
            ...
            <?php if ($_settings->_array[18][1]) {
                echo "<td class=td_background_top width=16 align=center>Do</td>";
            } ?>
        </tr>
        <?php
        for ($z = 1; $z < count($_monat->_MonatsArray); $z++) {
            ...
            if ($_settings->_array[15][1] == 1 || $_settings->_array[26][1]) {
                echo " <td ".$_monat->_MonatsArray[$z][30]." width=16 align=center><a target='_blank' href='?action=add_time&timestamp=".$_monat->_MonatsArray[$z][0].$modal."' title='Zeit hinzuf&uuml;gen'><img border='0' src='images/icons/time_add.png'></a></td>\n";
            }
            ...
            if ($_settings->_array[16][1] == 1 || $_settings->_array[26][1]) {
                echo " <td ".$_monat->_MonatsArray[$z][30]." width=16 align=center><a target='_blank' href='?action=add_time_list&timestamp=".$_monat->_MonatsArray[$z][0].$modal."' title='mehrere Zeiten hinzuf&uuml;gen'><img border='0' src='images/icons/time_go.png'></a></td>\n";
            }
            //-------------------------------------------------------------------------
        ...
        <tr>
            ...
            <td COLSPAN=<?php echo $t; ?> class=td_background_top width=550 align=left>Sollstunden:
                <?php
                echo $_monat->_SummeSollProMonat; ?> Std.
            </td>
            <td class=td_background_top width=40
                align=center><?php if ($_monat->_SummeWorkProMonat > 0) echo $_monat->_SummeWorkProMonat ?></td>
            ...
            <td COLSPAN=<?php echo $a; ?> class=td_background_top width=50
                align=center><?php echo $_monat->_SummeAbsenzProMonat ?></td>
            <?php
            if ($_settings->_array[18][1]) {
                echo "<td class=td_background_top width=16 align=center> </td>";
            }
            ?>
        </tr>
    </table>
