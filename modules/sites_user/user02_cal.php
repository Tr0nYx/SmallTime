...
if (strstr($_template->_bootstrap, 'true')) {
    ...
    foreach ($monate as $_month) {
        ...
        if ($i == $_time->_monat) {
            echo '		<li class="nav-item active"><a class="nav-link active" id="MonatAktiv" title="Monat '.$_month.'" href="?action=show_time&timestamp='.$_timestamp.'">'.$_month.'</a></li>';
        } else {
            echo '		<li class="nav-item"><a class="nav-link" id="Monat" title="Monat '.$_month.'" href="?action=show_time&timestamp='.$_timestamp.'">'.$_month.'</a></li>';
        }
        $i++;
    }
    echo '	</ul>';
