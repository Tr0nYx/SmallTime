    ...
    <ul class="nav nav-tabs adminmenu red accent-1">
        <?php if ($_settings->_array[13][1]) { ?>
            <li class="nav-item<?php echo $_action == "anwesend" ? ' active' : ''; ?>"><a
                        ...
                        href="?action=anwesend">Liste</a>
            </li> <?php } ?>
        <li class="nav-item<?php echo $_action == "show_time" ? ' active' : ''; ?>"><a
                    class="nav-link<?php echo $_action == "show_time" ? ' active' : ''; ?>" id="Home" title="Home"
                    href="?action=show_time">Home</a></li>
        <li class="nav-item<?php echo $_action == "show_year" ? ' active' : ''; ?>"><a
                    class="nav-link<?php echo $_action == "show_year" ? ' active' : ''; ?>" id="Jahr"
                    title="Jahres&uuml;bersicht"
                    href="?action=show_year">Jahr</a></li>
        <li class="nav-item<?php echo $_action == "show_pdf" ? ' active' : ''; ?>"><a
                    class="nav-link<?php echo $_action == "show_pdf" ? ' active' : ''; ?>" id="PDF" title="PDF anzeigen"
                    href="?action=show_pdf">PDF</a></li>
        <?php if ($_settings->_array[20][1] == 0) { ?>
            <li class="nav-item<?php echo $_action == "print_month" ? ' active' : ''; ?>">
                <a class="nav-link<?php echo $_action == "print_month" ? ' active' : ''; ?>" id="Drucken"
                   title="Monats&uuml;bersicht drucken"
                   href="?action=print_month&timestamp=<?php echo $_time->_timestamp ?>&print=0&calc=1">Drucken</a>
            </li>
        <?php } elseif ($_settings->_array[20][1] >= date("j", time())) { ?>
            <li class="nav-item<?php echo $_action == "print_month" ? ' active' : ''; ?>">
                <a class="nav-link<?php echo $_action == "print_month" ? ' active' : ''; ?>" id="Drucken"
                   title="Monats&uuml;bersicht vom letzten Monat drucken"
                   href="?action=print_month&timestamp=<?php echo $_time->_timestamp ?>&print=0">Drucken</a>
            </li>
        <?php } ?>
        <?php if ($_settings->_array[24][1]) { ?>
            <li class="nav-item<?php echo $_action == "design" ? ' active' : ''; ?>"><a
                        class="nav-link<?php echo $_action == "design" ? ' active' : ''; ?>" id="Design"
                        title="Design"
                        href="?action=design">Design</a></li>
        <?php } ?>
        <li class="nav-item<?php echo $_action == "password" ? ' active' : ''; ?>"><a
                    class="nav-link<?php echo $_action == "password" ? ' active' : ''; ?>" id="password"
                    title="password"
                    href="?action=password">Passwort</a></li>
        <li class="nav-item">
            <?php
            // ----------------------------------------------------------------------------
		...
		<tr>";
            $_startjahr = date("Y", trim($_user->_BeginnDerZeitrechnung));    // User - Einstellungen
